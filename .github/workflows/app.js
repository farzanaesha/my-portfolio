...
async function renderPdfPage(num) {
  ...
  pageWrap.style.cssText = `position:relative;width:${vp.width}px;height:${vp.height}px;box-shadow:0 2px 16px rgba(0,0,0,.15);flex-shrink:0;cursor:${hlMode?'crosshair':'default'};`;

  const canvas = document.createElement('canvas');
  canvas.width = vp.width; canvas.height = vp.height;
  canvas.style.cssText = 'display:block;position:absolute;top:0;left:0;';

  // Highlight overlay sits on top of canvas — no text layer needed
  const hlLayer = document.createElement('div');
  hlLayer.id = 'hl-layer';
  hlLayer.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:10;';

  // Drag-select capture layer (on top of everything, only active in hl mode)
  const dragLayer = document.createElement('div');
  dragLayer.style.cssText = `position:absolute;top:0;left:0;width:100%;height:100%;z-index:20;cursor:${hlMode?'crosshair':'default'};`;

  pageWrap.append(canvas, hlLayer, dragLayer);
  wrap.appendChild(pageWrap);

  await page.render({ canvasContext: canvas.getContext('2d'), viewport: vp }).promise;

  ge('pdf-page-info').textContent = `Page ${num} / ${pdfDoc.numPages}`;
  renderPageHighlights(num, hlLayer);
  setupDragSelect(dragLayer, hlLayer, num, 'pdf');
}
...
function renderPageHighlights(pageNum, hlLayer) {
  ...
  highlights.filter(h => h.page === pageNum && h.x !== undefined).forEach(h => {
    ...
    // Parse hex color and render as clearly visible semi-transparent overlay
...
function removeHighlight(id) {
  ...
  const hlLayer = ge('hl-layer');
  if (hlLayer) renderPageHighlights(pdfPage, hlLayer);
  applyTextHighlights();
  renderHlList();
  persistHighlights();
}
...
function toggleHighlight() {
  ...
  const btn = ge('hlbtn');
  if (hlMode) {
    btn.style.cssText = 'background:#FFE066;border:1px solid #c8a800;border-radius:3px;padding:5px 10px;font-family:\'JetBrains Mono\',monospace;font-size:10px;cursor:pointer;display:inline-flex;align-items:center;gap:4px;color:#000;';
    ge('pdf-canvas-wrap').style.cursor = 'crosshair';
  } else {
  ...
  // Re-render to toggle pointer-events on text layer
