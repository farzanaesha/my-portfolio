...
async function renderPdfPage(num) {
  ...
  // 1. Render PDF into canvas
  await page.render({ canvasContext: ctx, viewport: vp }).promise;

  // 2. Save clean copy into offscreen buffer for instant redraws during dragging
  _offscreen = document.createElement('canvas');
  _offscreen.width  = canvas.width;
  _offscreen.height = canvas.height;
  _offscreen.getContext('2d').drawImage(canvas, 0, 0);

  // 3. Paint any saved highlights on top
  drawHighlightsOnCanvas(ctx, num);

  ge('pdf-page-info').textContent = `Page ${num} / ${pdfDoc.numPages}`;

  // 4. Setup drag-to-highlight
  setupCanvasDrag(canvas, ctx, vp, num);
}

// Offscreen canvas stores the clean rendered PDF page
// Highlights are painted on top of it on the visible canvas
// This avoids async race conditions completely
let _offscreen = null;

function drawHighlightsOnCanvas(ctx, pageNum) {
  // First restore the clean PDF from offscreen buffer
  if (_offscreen) {
    ctx.drawImage(_offscreen, 0, 0);
  }
  // Then paint each highlight on top
  highlights.filter(h => h.page === pageNum && h.x !== undefined).forEach(h => {
    const hex = (h.color || '#FFE066').replace('#','');
    const r = parseInt(hex.slice(0,2),16)||255;
    const g = parseInt(hex.slice(2,4),16)||224;
...
function setupCanvasDrag(canvas, ctx, vp, pageNum) {
  ...
  function onMove(e) {
    ...
    const pos = getPos(e);
    // Restore clean page + highlights, then draw live selection box
  ...
  function onEnd(e) {
    ...
    if (w < 6 || h < 6) { drawHighlightsOnCanvas(ctx, pageNum); return; }
...
function renderPageHighlights(pageNum, hlLayer) {
  // For PDF: use offscreen buffer + draw highlights — no async needed
  ...
  // Fallback if offscreen not ready yet
  if (canvas && pdfDoc) {
    pdfDoc.getPage(pageNum).then(pg => {
      ...
      pg.render({ canvasContext: ctx, viewport: vp }).promise.then(() => {
        _offscreen = document.createElement('canvas');
        _offscreen.width = canvas.width; _offscreen.height = canvas.height;
        _offscreen.getContext('2d').drawImage(canvas, 0, 0);
        drawHighlightsOnCanvas(ctx, pageNum);
      });
    });
